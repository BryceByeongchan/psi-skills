...
Parse `sigma.out` files from a convergence sweep (from `psi-bgw-gw-conv-sigma` or `psi-bgw-gw-conv-epsilon`) and report a QP gap convergence table using the Eqp1 column.

## Usage

```
psi:bgw-gw-conv-analyze calcs:dir1,dir2,... vbm:ik=N,n=M cbm:ik=N,n=M sweep:param [threshold:0.05]
```
