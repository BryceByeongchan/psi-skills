#!/usr/bin/env python3
"""Pull calculation files from remote HPC via rsync."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Pull calc files from remote")
    ...
    if args.sync_all:
        _rsync(remote_base, f"{local_dir}/", hostname)
    else:
    ...
    print(f"Pulled {args.calc_id} from {hostname}:{hpc_path}")
