...
```
psi:qe-input-generator <structure_file> <calc_type> [calc_id:cNNN] [ecutwfc:NN] [kgrid:Nx,Ny,Nz] [pp_dir:path] [pp_template:pattern] [options...]
```

### Post-processing mode

```
psi:qe-input-generator <executable> [parent:cNNN] [calc_id:cNNN] [options...]
```
...
4. For any suggested flag not in the templates above, look up its official documentation using the validator's lookup mode:
   ...
   ```
   where `{validator_skill_dir}` is the `psi-qe-input-validator` skill directory (sibling of this skill's directory). Use the returned `info`, `options`, and `default` to present accurate suggestions.
...
- **When saving to a psi calc directory**, always use the `input/` subdirectory: `calc_db/{calc_id}/input/pw.in`.
